...
export function Copyright() {
  ...
  return (
    <div className='m-auto flex max-w-screen-lg flex-col items-center'>
      ...
      <small><Link aria-label={`Go to Github's ${ownerConfigs.name} page`} className='transition-all hover:text-brand-primary-hover' href={ownerConfigs.githubLink} target='_blank'>{ownerConfigs.user} | {ownerConfigs.name}</Link></small>
    </div>
  )
}
